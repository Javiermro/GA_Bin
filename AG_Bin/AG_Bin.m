...
psize = 1000;
ngen  = 100;
numit   = 1; % número de iteraciones de generación
probmut = 0.000001;%10.0; % probabilidad de mutacion máxima del 30% 
% porpas = 70; % 70 Porcentaje de individuos que pasan
% npass = 40; % número de individuos que pasan
nelit = 2; %round((0.02*psize) + 1) ; %cantidad de individuos elit 2% o el mejor
tol = 1e-5 ;
Sol = zeros(psize,1);
% Pop = zeros(nvars,psize);
...
while Best>tol;
  igen = igen + 1;
  for ipop=1:psize ;  Sol(ipop) = Func(Pop(ipop,:),nvars) ;   end
...
%%% CRUZAMIENTO (Crossover Simple) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  for icros=1:ncros
    ...
    ind2= ind1 ;
    while ind2 == ind1 ; ind2 = 1 + round((psize-nelit-1)*rand) ;  end   
    IndM = Pop(ind1,:) ;
    IndF = Pop(ind2,:) ;    
    
    if mod(igen,2)==0 %generacion par
        Pop(ind1,1:1:9)   = IndF(1:1:9);
        Pop(ind1,14:1:31) = IndF(14:1:31);

        Pop(ind2,1:1:9)   = IndM(1:1:9);
        Pop(ind2,14:1:31) = IndM(14:1:31);
    else %generación impar
        Pop(ind1,1:2:32)  = IndF(1:2:32);
        Pop(ind2,1:2:32)  = IndM(1:2:32);
    end
...
%         mut = probmut ;
%       end
%     if rand <= mut % se hace la mutación con un 30% de probabilidad
%         rp = mean(NewPob(ind,:)) ;
